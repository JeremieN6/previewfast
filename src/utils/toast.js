...
export function showToast({
  ...
  toast.id = toastId
  toast.className = 'flex w-full max-w-xs items-center rounded-lg bg-white p-4 text-gray-500 shadow-lg ring-1 ring-gray-200 animate-slide-in-right dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-700'
  toast.setAttribute('role', 'alert')

  // Icône selon le type
  const icons = {
    success: `
      <div class="inline-flex shrink-0 items-center justify-center w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-900 dark:text-green-200">
    ...
    `,
    error: `
      <div class="inline-flex shrink-0 items-center justify-center w-8 h-8 text-red-500 bg-red-100 rounded-lg dark:bg-red-900 dark:text-red-200">
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    ...
    `,
    warning: `
      <div class="inline-flex shrink-0 items-center justify-center w-8 h-8 text-orange-500 bg-orange-100 rounded-lg dark:bg-orange-900 dark:text-orange-200">
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    ...
    `,
    info: `
      <div class="inline-flex shrink-0 items-center justify-center w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-900 dark:text-blue-200">
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
  ...
  let toastHTML = `
    ...
    <div class="ms-3 text-sm font-medium text-gray-900 flex-1 dark:text-white">${message}</div>
  `
  ...
  if (action && onAction) {
    toastHTML += `
      <button 
        type="button" 
        class="ms-2 text-sm font-semibold text-blue-600 hover:text-blue-800 hover:underline dark:text-blue-400 dark:hover:text-blue-300"
        data-action="true"
      >
        ${action}
      </button>
  ...
  toastHTML += `
    <button 
      type="button" 
      class="ms-auto -mx-1.5 -my-1.5 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-900 focus:ring-2 focus:ring-gray-300 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
      data-dismiss="true"
      aria-label="Close"
    >
